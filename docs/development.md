...
├── Pulse.Domain/                 # Core entities and interfaces
│   ├── Entities/
...
├── Pulse.Application/            # CQRS commands, queries, DTOs
│   ├── Commands/
...
├── Pulse.Infrastructure/         # EF Core, repositories, Win32 tracking
│   ├── Data/
│   │   └── PulseDbContext.cs     # DbContext with entity configs and seed data
│   ├── Repositories/
│   │   └── Repositories.cs       # All repository implementations
│   └── Services/
│       └── ActiveWindowService.cs # Win32 P/Invoke window tracker
├── Pulse.UI/                     # WPF presentation layer
...
1. Create the entity class in `Pulse.Domain/Entities/`
2. Add a `DbSet<>` in `PulseDbContext` and configure it in `OnModelCreating`
3. Define a repository interface in `Pulse.Domain/Interfaces/`
4. Implement the repository in `Pulse.Infrastructure/Repositories/`
5. Register it in `App.xaml.cs` DI container

### Adding a New CQRS Command/Query

1. Create the record class implementing `IRequest<T>` in `Pulse.Application/Commands/` or `Queries/`
...
1. Create a ViewModel in `Pulse.UI/ViewModels/` extending `ObservableObject`
2. Create a XAML `UserControl` in `Pulse.UI/Pages/`
...
dotnet publish Pulse.UI/Pulse.UI.csproj -c Release -o ./publish
```
